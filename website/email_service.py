...
def send_promotion_50off_email(user):
    """Send 50% OFF promotion email to free users"""
    try:
        # Create email message
        msg = Message(
            subject="🎉 Special Offer: 50% OFF of all product - LetterForLater",
            recipients=[user.email],
            sender=os.getenv('MAIL_USERNAME', 'support@letterforlater.com')
        )
        
        # Render HTML template
        msg.html = render_template('emails/promotion_50off.html',
            user_name=f"{user.first_name} {user.last_name}",
            user_email=user.email,
        ...
        # Render text template
        msg.body = render_template('emails/promotion_50off.txt',
            user_name=f"{user.first_name} {user.last_name}",
            user_email=user.email,
            pricing_url="https://letterforlater.com/pricing",
            dashboard_url="https://letterforlater.com"
        ...
        # Send email using safe_send_email for rate limiting and error handling
        success = safe_send_email(msg, email_type='promotion_50off')
        
        if success:
            print(f"✅ 50% OFF promotion email sent to {user.email}")
            return True
        else:
            print(f"❌ Failed to send 50% OFF promotion email to {user.email} (rate limited or error)")
            return False
        
    except Exception as e:
        print(f"❌ Error sending 50% OFF promotion email: {str(e)}")
        import traceback
        traceback.print_exc()
        return False
